...
def install():
    """
    Устанавливает бота при установке приложения из Bitrix.
    Bitrix обращается к этому пути при установке приложения.
    """
    print("\n⚙️ Установка приложения из Bitrix...")
    print("Параметры:", request.args.to_dict())

    # При установке сразу регистрируем бота через REST вебхук
    new_id = register_bot()
    if new_id:
        return jsonify({
            "ok": True,
